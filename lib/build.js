...
const build = async () => {
  ...
  if (middlewares.length > 0) {
    logger.info('Pre-build middleware execution');
    const promises = [];
    middlewares.filter((middleware) => middleware.type === 'prebuild').forEach((middleware) => {
  ...
  pages.forEach((page, index) => {
    page.attributes.forEach((attributes) => {
      const baseExportPath = normalizePath(`${getPathPropertyUrl(page.urlPath, attributes.pathParams)}`);
      const localizedExportpath = normalizePath(`/${attributes.localization.locale}${getPathPropertyUrl(page.urlPath, attributes.pathParams)}`);

      const baseUrl = `${getPathPropertyUrl(page.urlPath, attributes.pathParams)}`;
      const localizedUrl = `/${attributes.localization.locale}${getPathPropertyUrl(page.urlPath, attributes.pathParams)}`;
      ...
      if (attributes.localization.locale === defaultLocale) {
        tasks.push({
          ...
          data: {
            pageIndex: index,
            exportPath: `${baseExportPath}.html`,
      ...
      tasks.push({
        ...
        data: {
          pageIndex: index,
          exportPath: `${localizedExportpath}.html`,
          localizedExportPath,
          attributes: {
            hostname,
            path: localizedUrl,
      ...
      sitemap.push(`${hostname}${baseUrl}`);
      sitemap.push(`${hostname}${localizedUrl}`);
    });
  });
  ...
  if (middlewares.length > 0) {
    logger.info('Post-build middleware execution');
    const promises = [];
    middlewares.filter((middleware) => middleware.type === 'postbuild').forEach((middleware) => {
      promises.push(middleware.default());
    });
    await Promise.all(promises);
  }
