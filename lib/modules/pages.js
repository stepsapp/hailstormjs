...
export const { defaultLocale, locales, hostname } = config;
...
export const getstaticPathParams = async (module, locale) => {
  ...
  if (typeof module.staticPathParams === 'undefined') {
    items.push({ localization: { locales, locale }, pathParams: {} });
  } else {
    const staticPathParams = await module.staticPathParams({ defaultLocale, locale });
    staticPathParams.forEach((item) => {
      items.push({ localization: { locales, locale }, pathParams: item });
    });
  }
  return items;
};
...
export const prepareLocalizationObject = (localizationObject, basePath, localizedUrl) => {
  const localization = localizationObject;
  localization.path = localizedUrl;
  localization.basePath = basePath;
  return localization;
};
